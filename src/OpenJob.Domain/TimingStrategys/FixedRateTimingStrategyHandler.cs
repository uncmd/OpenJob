...
public class FixedRateTimingStrategyHandler : TimingStrategyHandlerBase
{
    ...
    public override void Validate(string timeExpression)
    {
        ...
        {
            throw new OpenJobException($"{errorMessage} invalid timeExpression!");
        }

        if (delay < 0)
        {
            throw new OpenJobException($"{errorMessage} the rate must be greater than 0 ms");
        }
        ...
        {
            throw new OpenJobException($"{errorMessage} the rate must be less than {maxInterval} ms");
        }
    }
